...
import { OneExpenses, addExpenses } from "../../redux/Expenses";
...
const EditExpenses = (props) => {
  ...
  console.log(oneExpenses.name)
  const colors = tokens(theme.palette.mode);
  const isNonMobile = useMediaQuery("(min-width:600px)");
  const { sidebarRTL } = useSidebarContext();
  const loading = useSelector((state) => state.selectMenu.Expenses);
  
  const handleClickOpen = async () => {
    await dispatch(OneExpenses(props.id))
      ...
      >
        <DialogContent dir={sidebarRTL ? "rtl" : "ltr"}>
          ...
          <form onSubmit={formik.handleSubmit} dir={sidebarRTL ? "ltr" : "rtl"}>
            ...
            <CustomSelect options={expensesMenu} onChange={setExpensesValue} />
            <CustomDialogActions
              onClick={handleClose}
              onClickAction={formik.handleSubmit}
              text={loading ? t("wait") : t("Edit")}
            />
          </form>
        </DialogContent>
      </Dialog>
